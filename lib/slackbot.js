...
Slackbot.prototype._forwardInTheDedicatedChannel = function(originalMessage) {
    ...
    console.log("team info = " + JSON.stringify(self.getTeamInfo(self.settings.my_token)) + "\n settings = " + JSOON.stringify(self.settings))
    var attach = [{
        "fallback": "Sorry, an audition problem occured!.",
        "color": "#3AA3E3",
        "text": originalMessage.text,
        "author_name": sender.name,
        "author_link": "https://" + self.getTeamInfo(self.settings.my_token).domain + ".slack.com/messages/@" + sender.name + "/team/" + sender.name + "/",
    ...
    var attach2 = [{
        ...
        "actions": [{
            "name": "activate",
            "text": "Turn turn me On if I seems away",
            "type": "button",
            "value": "activate"
        }, ]
    }]
