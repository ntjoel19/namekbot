...
Slackbot.prototype._forwardInTheDedicatedChannel = function(originalMessage) {
    ...
    var team_info = this.getTeamInfo({ token: self.settings.me_token })
    console.log("team info = " + JSON.stringify(team_info))
    var attach = [{
        "fallback": "Sorry, an audition problem occured!.",
        "color": "#3AA3E3",
        "text": originalMessage.text,
        "author_name": sender.name,
        "author_link": "https://" + team_info.team.name || "duo-code-temp" + ".slack.com/messages/@" + sender.name + "/team/" + sender.name + "/",
