...
Slackbot.prototype._forwardInTheDedicatedChannel = function(originalMessage) {
    ...
    console.log("team info = " + JSON.stringify(self.getTeamInfo({ token: self.settings.me_token })) + "\n settings = " + JSON.stringify(self.settings))
    var attach = [{
        "fallback": "Sorry, an audition problem occured!.",
        "color": "#3AA3E3",
        "text": originalMessage.text,
        "author_name": sender.name,
        "author_link": "https://" + self.getTeamInfo({ token: self.settings.me_token }).team.name + ".slack.com/messages/@" + sender.name + "/team/" + sender.name + "/",
